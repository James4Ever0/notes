...
When you encounter a Redis instance and you quickly want to learn about the setup you just need a few simple commands to peak into the setup. Of course it doesnât hurt to look at the official full command documentation, but below is a listing just for sysadmins.
Accessing Redis
CLI

First thing to know is that you can use âtelnetâ (usually on Redis default port 6379)
...
Redis Lists	lrange <key> <start> <stop> Accessing lists
 	...
 	lpush mylist "value" Push âvalueâ to list
 	lpush mylist 5 Push number 5 to list
 	rpush mylist "value" Push âvalueâ to beginning (unshift)
 	...
 	lrem mylist 1 "value" Remove âvalueâ count times
...
Working with Redis Hashes	hexists myhash field1 Check if hash key exists
 	...
 	hset myhash field1 "value" Set key with âvalueâ
...
For scripting just pass commands to âredis-cliâ. For example:
...
The statistics command is âINFOâ and will give you an output as following.
...
Note that keys and values are alternating and you can change each key by issuing a âCONFIG SETâ command like:
...
Redis has a concept of separated namespaces called âdatabasesâ. You can select the database number you want to use with âSELECTâ. By default the database with index 0 is used. So issuing

redis 127.0.0.1:6379> SELECT 1
OK
redis 127.0.0.1:6379[1]>

switches to the second database. Note how the prompt changed and now has a â[1]â to indicate the database selection. To find out how many databases there are you might want to run redis-cli from the shell:
...
and watch for the âroleâ line which shows either âmasterâ or âslaveâ. Starting with version 2.8 the âINFOâ command also gives you per slave replication status looking like this
...
When running this command Redis will fork and the new process will dump into the âdbfilenameâ configured in the Redis configuration without the original process being blocked. Of course the fork itself might cause an interruption. Use âLASTSAVEâ to check when the dump file was last updated. For a simple backup solution just backup the dump file. If you need a synchronous save run âSAVEâ instead of âBGSAVEâ.
Listing Connections
...
The propably most useful command compared to memcached where you need to trace network traffic is the âMONITORâ command which will dump incoming commands in real time.
...
additionally use âSLOWLOGâ to track the slowest queries in an interval. For example
