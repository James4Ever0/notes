...
modified: '2022-08-12T13:15:44.464Z'
---
...
- need manual kernel module loading before launching the n2n edge command.
- when running `` as root, python library path need to be appended to `sys.path`, though `where_is_kali.py` does not have 3rd party libs so running it is fine without the patch.
